...
<html lang="es">
	...
	<body>
		<section>
			<article>
				<form action="./logs_usuario_tvsaint.php" method="POST" id="logsusu" name="logsusu" enctype="multipart/form-data"> 
				<ul>
					<li>
			...
			<article>
				...
				if($_POST && strlen($_POST['buscar'])>0) {
					...
					for($z=0;$z<count($arrtfn);$z++){
						echo "<li><b>" . $arrtfn[$z] . "</b></li>";
						for ($b=$axoini; $b <= $axofin; $b++) { 
							for($a=1;$a<=12;$a++){
								if (strlen($a)==1) { $c="0".$a; } else { $c=$a; }
								$url = "/nfscluster/televisainternacional/logs/".$arrtfn[$z]."_" . $b . $c .".log";
								if(file_exists($url)){
